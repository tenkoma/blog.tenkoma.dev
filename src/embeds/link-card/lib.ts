...
const safeGetHTML = makeSafeGetter<ReturnType<typeof load>>(async (res) => load(await res.text()));

/** Helper to get the `content` attribute of an element. */
const getContent = (el: ReturnType<ReturnType<typeof load>>) => el?.attr("content") ?? null;
...
export async function parseOpenGraph(pageUrl: string) {
  ...
  const getMetaProperty = (prop: string) => getContent($(`meta[property=${JSON.stringify(prop)}]`));
  const getMetaName = (name: string) => getContent($(`meta[name=${JSON.stringify(name)}]`));
  ...
  const url =
    urlOrNull(getMetaProperty("og:url") || $("link[rel='canonical']").attr("href") || null) ||
    pageUrl;
